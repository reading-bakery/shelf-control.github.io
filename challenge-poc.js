...
document.addEventListener("DOMContentLoaded", () => {
    const csvUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTXx02YVtknMhVpTr2xZL6jVSdCZs4WN4xN98xmeG19i47mqGn3Qlt8vmqsJ_KG76_TNsO0yX0FBEck/pub?gid=1234038788&single=true&output=csv";
    const container = document.querySelector(".poc-challenge");
    ...
    function createCard(item) {
        ...
        if (hasCover) {
            ...
            div.innerHTML = `<div class="poc-placeholder"></div>`;
        }

        // Buchstaben NUR hinzufügen, wenn ein Cover existiert
        if (hasCover) {
            const letterContainer = document.createElement("div");
            letterContainer.classList.add("poc-letter-container");
            
            const numText = String(item.Nummer || "");
            [...numText].forEach(letter => {
                const span = document.createElement("span");
                span.classList.add("poc-letter");
    ...
    function createCarousel(data, maxPerRow, maxRows) {
        ...
        carousel.classList.add("poc-carousel");

        // Carousel Navigation
        const nav = document.createElement("div");
        nav.classList.add("poc-carousel-nav");
        for (let i = 0; i < totalSlides; i++) {
            const dot = document.createElement("span");
            dot.classList.add("poc-dot");
        ...
        track.classList.add("poc-carousel-track");

        for (let i = 0; i < totalSlides; i++) {
            const slide = document.createElement("div");
            slide.classList.add("poc-carousel-slide");
